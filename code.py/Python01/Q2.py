class BST : 
    ...
    def search(self,data) :
        if self.key == data :
            print("Node({}) is found!".format(data))
            return
        if data < self.key :
            if self.lchild:
                self.lchild.search(data)
            else :
                print("Node({}) is not present in tree!".format(data))
        else :
            if self.rchild:
...
print("node with smallest key is {}".format(result_1))
result_2 = root.max_node()
print("node with maximum key is {}".format(result_2))
